...
def main():
    ...
    checks = [
        ("Incremental k-means works",
         True, "core algorithm with tie-handling"),
        ("Batch k-means works",
         True, "Lloyd's algorithm"),
        ("A/B accuracy near 87%",
         abs(ab['incremental_accuracy'] - 0.87) < 0.15,
         f"got {ab['incremental_accuracy']*100:.1f}%"),
        ("Mixture SE near 2.8",
         abs(mix['standard_error'] - 2.8) < 2.0,
         f"got {mix['standard_error']:.2f}"),
        ("Mixture accuracy near 96%",
         abs(mix['accuracy'] - 0.96) < 0.15,
         f"got {mix['accuracy']*100:.1f}%"),
        ("Order sensitivity reasonable",
         max_var_pct < 20,
         f"{max_var_pct:.1f}% variation"),
        ("C/R changes k",
         cr.k_final != 6,
         f"went from 6 to {cr.k_final}"),
        ("Scrambled test detects dependence",
         hs_result['ratio'] > 1.2,
         f"ratio={hs_result['ratio']:.2f}x"),
    ]
